    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          <ul className="flex items-center gap-1 text-sm font-medium">
            {navLinks.map((link) => (
              <motion.li key={link.name} whileHover={{ y: -2 }} whileTap={{ scale: 0.95 }}>
                <a
                  href={link.href}
                  className="px-4 py-2 rounded-lg text-slate-600 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 hover:bg-slate-100/80 dark:hover:bg-white/5 transition-all duration-300 relative group"
                >
                  {link.name}
          ...
          <div className="flex items-center gap-2 border-l border-slate-200/50 dark:border-white/5 pl-4">
            <motion.button
              whileHover={{ scale: 1.1, rotate: 180 }}
              whileTap={{ scale: 0.9 }}
              onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
              className="p-2 rounded-lg text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100/80 dark:hover:bg-white/5 transition-all duration-300"
            >
